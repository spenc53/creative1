...
$(document).ready(() => {
  ...
  function getScores() {
    ...
    var url = 'http://35.166.135.246:3000/';
    var method = 'GET';
    var xhr = createCORSRequest(method, url);

    xhr.onload = function () {
      let data = JSON.parse(xhr.response.replace(/'/g, '"'));
      $('#high-scores').empty();
      for (let i in data) {
        let item = data[i]
        $('#high-scores').append("<li>" + item['name'] + ' : ' + item['score'] + '</li>');
      }
    };

    xhr.onerror = function () {
      $('#high-scores').empty().text("There was a problem");
    };
  ...
  $('#submit-score').click( () => {
    ...
    xhr.send("{'name':'" + $('#username').val() + "','score':" + STATE.ACTUAL_SCORE + "}");
  })
